        ...
        if j%2 == 1:
            for i in range(nx//2 + 1):
                for k in [kerf/2, -kerf/2]:
                    start_pos = (max(0., cell_width*(2*i - 1) + gap/2) + offset_x, cell_height*j + k + buffer_height + offset_y)
                    end_pos = (min(width, cell_width*(2*i + 1) - gap/2) + offset_x, cell_height*j + k + buffer_height + offset_y)
                    p.add_line(start_pos, end_pos)
                if i != 0:  # Add left arc
                    center = (max(0., cell_width*(2*i - 1) + gap/2) + offset_x, cell_height*j + buffer_height + offset_y)
                    p.add_circle(center, kerf/2, start_angle=-np.pi/2, end_angle=-3*np.pi/2)
                if i != (nx//2):  # Add right arc
                    center = (min(width, cell_width*(2*i + 1) - gap/2) + offset_x, cell_height*j + buffer_height + offset_y)
    ...
    for i in range(left, right):
        if i%2 == 0:
            for j in range(ny//2 + 1):
                for k in [kerf/2, -kerf/2]:
                    start_pos = (cell_width*i + k + offset_x, max(0., cell_height*(2*j - 1) + gap/2) + buffer_height + offset_y)
                    end_pos = (cell_width*i + k + offset_x, min(height, cell_height*(2*j + 1) - gap/2) + buffer_height + offset_y)
                    p.add_line(start_pos, end_pos)
                if True:  # Add bottom arc
                    center = (cell_width*i + offset_x, max(0., cell_height*(2*j - 1) + gap/2) + buffer_height + offset_y)
                    p.add_circle(center, kerf/2, start_angle=-np.pi, end_angle=0)
                if True:  # Add top arc
                    center = (cell_width*i + offset_x, min(height, cell_height*(2*j + 1) - gap/2) + buffer_height + offset_y)
                    p.add_circle(center, kerf/2, start_angle=np.pi, end_angle=0)
        else:
            for j in range(ny//2):
                for k in [kerf/2, -kerf/2]:
...
if __name__ == '__main__':
    ...
    now = datetime.now()
    name_clarifier = "_halfsquare_halfrectangular_pattern_nx={:d}xny={:d}_wx={:.2f}xwy={:.2f}_kerf={:.2f}_gap={:.2f}_noseamholes".format(
    ...
    # Add the vertical cuts in the boundary between the square and rectangular sections
    # i = nx
    # for j in range(ny//2 + 1):
    #     for k in [kerf/2, -kerf/2]:
