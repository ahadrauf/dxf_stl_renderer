...
def generate_squarelv1_pattern(p: PCBPattern, width, height, nx, ny, buffer_width, buffer_height, kerf, gap):
    ...
    left_edge_x = kerf/2  # -x_buffer
    right_edge_x = width - kerf/2  # + x_buffer
    bottom_edge_y = kerf/2  # -x_buffer  #  -buffer_height_pins
    top_edge_y = height - kerf/2  # + x_buffer
        ...
        for i in range(1, nx):
            if i%2 == 0:
                ...
                for j in range(ny//2 + 1):
                    for k in [kerf/2, -kerf/2]:
                        start_pos = (cell_width*i + k, max(bottom_edge_y, cell_height*(2*j - 1) + gap_eff/2))
                        end_pos = (cell_width*i + k, min(top_edge_y, cell_height*(2*j + 1) - gap_eff/2))
                        if j == (ny // 2):
                            end_pos = (cell_width*i + k, top_edge_y)
                        p.add_graphic_line([start_pos, end_pos], layer=layer)
                    if j != 0:  # Add bottom arc
                        center = (cell_width*i, max(bottom_edge_y, cell_height*(2*j - 1) + gap_eff/2))
    ...
    for layer in ["Edge.Cuts"]:
        ...
        for i in range(0, nx + 1, 2):
            ...
            if i != nx:
                ...
                x_edges.append(cell_width*i + 4*cell_width/3)
        x_edges.append(right_edge_x)
        x_edges = x_edges[1:-1]
        print(x_edges)

        # Add cut edges
        for i in range(1, len(x_edges) - 1, 2):
            p.add_graphic_line([(max(left_edge_x, x_edges[i] + cutaway_gap), bottom_edge_y),
                                (min(right_edge_x, x_edges[i + 1] - cutaway_gap), bottom_edge_y)], layer=layer)
            print(left_edge_x, x_edges[i] + cutaway_gap, right_edge_x, x_edges[i + 1] - cutaway_gap)
        ...
        for i in range(1, len(x_edges) - 1, 2):
            p.add_graphic_line([(max(left_edge_x, x_edges[i] + cutaway_gap), top_edge_y),
                                (min(right_edge_x, x_edges[i + 1] - cutaway_gap), top_edge_y)], layer=layer)
        for j in range(0, len(y_edges) - 1, 2):
    ...
    # Add seamholes
    for i in range(nx):
        p.add_graphic_arc((cell_width*(i + 1/2), cell_height*(ny - 1/2)), radius=0.4, start_angle=0, end_angle=2*np.pi, N=10)
        p.add_graphic_arc((cell_width*(i + 1/2), cell_height/2), radius=0.4, start_angle=0, end_angle=2*np.pi, N=10)
...
def generate_square_wiring(p: PCBPattern, width, height, nx, ny, buffer_height, kerf, gap):
    ...
    for i in range(nx):
        ...
        # pts = p.offset_trace([pin_loc, (pin_loc[0], kerf/2 + pad_spacing_y/2 + 0.1)], w=trace_width)
        ...
        for n in range(ny - 1):
            ...
            # pts = p.generate_rounded_curve(pts, trace_radius, N_corner)
            # pts = p.offset_trace(pts, w=trace_width)
...
if __name__ == '__main__':
    ...
    pad_spacing_x = trace_width*(ny_orig//2) + trace_spacing*(ny_orig//2 - 1) + 2*BOARD_EDGE_SPACING_EFF
    kerf = 3.  # mm
    gap = 1.5  # pad_spacing_x  # mm, for defining the straight line segments
    ...
    now = datetime.now()
    name_clarifier = "_pcb_square_simplesimulation"
    timestamp = now.strftime("%Y%m%d_%H_%M_%S") + name_clarifier
    ...
    p.generate_dxf(dxf_cut_filename, dxf_etch_filename, save_cut=True, save_etch=False, offset_x=x_buffer, offset_y=buffer_height,
                   include_traces_etch=False, cut_layers=("Edge.Cuts", "Eco2.User"), etch_layers=("F.Cu", "Edge.Cuts", 'Eco2.User'))
