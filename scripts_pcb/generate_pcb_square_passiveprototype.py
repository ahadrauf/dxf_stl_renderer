...
def generate_squarelv1_pattern(p: PCBPattern, width, height, nx, ny, buffer_width, buffer_height, kerf, gap):
    ...
    for layer in ["Edge.Cuts"]:
        ...
        for i in range(0, len(x_edges) - 1, 2):
            p.add_graphic_line([(max(p0[0], x_edges[i] + cutaway_gap), bottom_edge_y),
                                (min(p1[0], x_edges[i + 1] - cutaway_gap), bottom_edge_y)], layer=layer)
...
if __name__ == '__main__':
    ...
    kerf = 3.  # mm
    gap = 0.5  # pad_spacing_x  # mm, for defining the straight line segments
    ...
    now = datetime.now()
    name_clarifier = "_pcb_square_passiveprototype"
    timestamp = now.strftime("%Y%m%d_%H_%M_%S") + name_clarifier
